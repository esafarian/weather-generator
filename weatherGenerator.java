...
public class WeatherGenerator {
   ...
   int month, double[][] drywet, double[][] wetwet) {
	   ...
	   double wetwetProbability = 0.49;
	   double drywetProbability = 0.50;
	   
	   int[] forecast = new int[numberOfDaysInMonth[month]];
	   
	   
	   for(int i = 2; i < wetwet.length; i++) {
		   for (int j = 0; j < wetwet[i].length; j++) {
			  ...
			  if(wetwet[i][j] <= 0.49) { 
			   forecast[i] = StdRandom.uniform(0, 1);
				 ...
				 for(int x = 2; x < drywet.length; x++) {
					  for (int y = 0; y < drywet[x].length; y++) {
						   
						 if(drywet[x][y] >= 0.50) { 
						  forecast[x] = StdRandom.uniform(0, 1);
						 }	  
					  }
				 	}
   ...
   public static void main (String[] args) {
       ...
       for ( int i = 0; i < forecast.length; i++ ) {
           ...
           // ternary operator. (conditional) ? executed if true : executed if false
